...
const HTML_PATH = resolve(__dirname, "../packages/ui-web/index_c.html");

test("index_c.html loads main.js as a static module script (not dynamically)", () => {
  ...
  assert.ok(
    ...
    "index_c.html must use a static <script type=\"module\" src=\"./src/main.js\"> tag",
  );

  // Must NOT fall back to the old health-check dynamic-loading pattern.
  assert.ok(
    !html.includes("document.createElement('script')"),
    "index_c.html must not dynamically create a script element for main.js",
  );

  assert.ok(
    !html.includes("retryLoad"),
    "index_c.html must not contain the old retryLoad health-check loop",
  );
});
