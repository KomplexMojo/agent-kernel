...
test("affinity stack cost formula: 10 + 8·(n-1)² (design §6.2)", async () => {
  const { computeStackCost, computeCumulativeStackCost } = await import("../../packages/runtime/src/personas/configurator/cost-model.js");
...
test("vital max costs: 2H + 2M + S + 2D (design §7)", async () => {
  const { calculateVitalCost } = await import("../../packages/runtime/src/personas/configurator/cost-model.js");
...
test("regen costs are quadratic: 12·R² + 5·R² + 4·R² + 10·R² (design §8)", async () => {
  const { calculateRegenCost } = await import("../../packages/runtime/src/personas/configurator/cost-model.js");
...
test("affinity base cost is 30 (design §6.1)", async () => {
  const { COST_DEFAULTS, calculateAffinityCost } = await import("../../packages/runtime/src/personas/configurator/cost-model.js");
...
test("expression costs: external=35, async internal=25 (design §6.4)", async () => {
...
test("motivation costs: simple=25, async advanced=50 (design §6.6)", async () => {
  const { COST_DEFAULTS } = await import("../../packages/runtime/src/personas/configurator/cost-model.js");
...
test("minimum external affinity package cost: 30 + 10 + 35 = 75 (design §6.5)", async () => {
  const { COST_DEFAULTS, computeCumulativeStackCost } = await import("../../packages/runtime/src/personas/configurator/cost-model.js");
...
test("minimum internal affinity package cost: 30 + 10 + 25 = 65 (design §6.5)", async () => {
  const { COST_DEFAULTS, computeCumulativeStackCost } = await import("../../packages/runtime/src/personas/configurator/cost-model.js");
