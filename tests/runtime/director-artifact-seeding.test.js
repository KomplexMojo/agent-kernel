...
// Test 1: high-hazard (affinity) rooms receive artifact_proposal effects with
// budget proportional to dungeonBreakdown.artifacts
const highHazardScript = `
...
// budgetCeiling = floor(100 / 2) = 50 for each room
...
// vitals array should be present with health delta matching budgetCeiling
...
// personaRef should be present
assert.equal(fireProposal.personaRef, "director");

// planRef should be present
...
// Non-affinity room (rest) should produce no proposal
...
// Total artifact spend must not exceed dungeonBreakdown.artifacts (100)
...
test("director emits artifact_proposal effects for affinity rooms proportional to dungeonBreakdown.artifacts", () => {
  runEsm(highHazardScript);
});

// Test 2: no dungeonBreakdown → default delta used, proposals still emitted for affinity rooms
const noBudgetScript = `
import assert from "node:assert/strict";
import { createDirectorPersona } from ${JSON.stringify(personaModule)};
...
test("director emits artifact_proposal with zero budget when dungeonBreakdown.artifacts is absent", () => {
  runEsm(noBudgetScript);
});

// Test 3: rooms can override vitals and permanent flag per room
const customVitalsScript = `
import assert from "node:assert/strict";
import { createDirectorPersona } from ${JSON.stringify(personaModule)};
...
test("director uses room-level artifactVitals and artifactPermanent overrides in artifact_proposal", () => {
  runEsm(customVitalsScript);
});
