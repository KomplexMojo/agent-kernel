...
const captureModule = moduleUrl("packages/runtime/src/personas/orchestrator/llm-capture.js");

test("buildRuntimeDecisionEnvelope normalizes actor context and candidate actions", () => {
  const script = `
...
} from ${JSON.stringify(runtimeDecisionModule)};
...
test("runtime decision provider policy only allows live Ollama in explicit manual mode", () => {
  const script = `
import assert from "node:assert/strict";
import {
  RUNTIME_DECISION_LLM_LIVE_MODE,
...
test("captured LLM runtime decision resolves to an action without a live provider", () => {
  const script = `
import assert from "node:assert/strict";
import { buildRuntimeDecisionEnvelope, resolveActionFromLlmCapture } from ${JSON.stringify(runtimeDecisionModule)};
import { buildLlmCaptureArtifact } from ${JSON.stringify(captureModule)};
