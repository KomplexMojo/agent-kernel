...
test("layout hierarchy keeps preview and gameplay workspaces active with actor inspector rail", () => {
  const html = readHtml();
  assert.match(html, /\.workspace\s*\{[^}]*grid-template-columns:\s*1fr/);
  assert.match(html, /\.workspace\[data-active-tab="preview"\],\s*\.workspace\[data-active-tab="gameplay"\]\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s*minmax\(260px,\s*340px\)/);
  assert.match(html, /\.workspace:not\(\[data-active-tab="preview"\]\):not\(\[data-active-tab="gameplay"\]\)\s*#actor-inspector\s*\{[^}]*display:\s*none/);
  ...
  assert.match(html, /@media\s*\(min-width:\s*1680px\)\s*and\s*\(min-height:\s*900px\)[\s\S]*?\.workspace\[data-active-tab="preview"\],\s*\.workspace\[data-active-tab="gameplay"\]\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s*minmax\(320px,\s*420px\)/);
  ...
  assert.doesNotMatch(html, /\.simulation-shell\s*\{/);
  assert.doesNotMatch(html, /\.simulation-stage\s*\{/);
  assert.doesNotMatch(html, /#preview-frame-buffer\s*\{/);
  assert.doesNotMatch(html, /#preview-render-canvas\s*\{/);
});
