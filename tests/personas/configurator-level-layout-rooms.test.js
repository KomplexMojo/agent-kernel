...
test("room layouts are deterministic and connected for tiered sizes", async () => {
  const { normalizeLevelGenInput } = await import(
    "../../packages/runtime/src/personas/configurator/level-gen.js"
  );
  const { generateGridLayout } = await import(
