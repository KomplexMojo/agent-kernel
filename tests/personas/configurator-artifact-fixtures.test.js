...
const simConfig = readFixture("sim-config-artifact-v1-configurator-hazard.json");
const initialState = readFixture("initial-state-artifact-v1-configurator-affinity.json");

test("configurator artifact fixtures include hazards and affinity traits", () => {
  ...
  assert.equal(simConfig.layout.data.hazards.length, 1);
  assert.deepEqual(simConfig.layout.data.hazards[0].affinity, {
    kind: "fire",
    expression: "push",
    stacks: 2,
  });
  // Updated 2026-07-10: hazard coordinates adjudicated as room-relative (M3); formerly pinned grid-absolute semantics.
  // The regenerated fixture maps the authored hazard (2,2) into room R1 at (1,1) -> absolute (3,3).
