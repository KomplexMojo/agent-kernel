...
const catalogFixture = JSON.parse(readFileSync(resolve(__dirname, "../fixtures/pool/catalog-basic.json"), "utf8"));

test("budget loop repairs feasibility failures via repair prompt", async () => {
...
test("budget loop benchmarks walkability budget scaling up to 550000 at 1M total budget", async () => {
  const { runLlmBudgetLoop } = await import(
    "../../packages/runtime/src/personas/orchestrator/llm-budget-loop.js"
  );
...
test("budget loop supports one billion token budgets without imposed ceiling", async () => {
  const { runLlmBudgetLoop } = await import(
    "../../packages/runtime/src/personas/orchestrator/llm-budget-loop.js"
  );
...
test("budget loop avoids full-grid feasibility slowdown for very large walkability counts", async () => {
