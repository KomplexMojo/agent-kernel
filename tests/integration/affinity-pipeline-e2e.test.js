 ...
 *   runtime (core-setup) → bindings-ts → core-as (WASM) → ui-web (tile-affinity-visuals)
 */
...
const coreSetupUrl = moduleUrl("packages/runtime/src/runner/core-setup.mjs");
const bindingsUrl = moduleUrl("packages/bindings-ts/src/index.js");
const tileVisualsUrl = moduleUrl("packages/ui-web/src/views/tile-affinity-visuals.js");
const wasmUrl = moduleUrl("build/core-as.wasm");

const script = `
import assert from "node:assert/strict";
import { loadCore, readAffinityFieldAt, AFFINITY_KIND_BY_CODE } from ${JSON.stringify(bindingsUrl)};
import { initializeCoreFromArtifacts } from ${JSON.stringify(coreSetupUrl)};
import { deriveTileAffinityVisuals } from ${JSON.stringify(tileVisualsUrl)};
...
// ── Step 2: Read field records from the WASM core ──

// Collect field records for all tiles in the grid, all 10 kinds
...
assert.ok(fieldRecords.length > 0, "field records collected from WASM");
...
const tileVisuals = deriveTileAffinityVisuals({
  fieldRecords,
  resourceBundle: { mappings: { overlays: {} } },
});

assert.ok(tileVisuals instanceof Map, "tileVisuals is a Map");
assert.ok(tileVisuals.size > 0, "tileVisuals has entries");

// Verify origin tile visual
const originVisual = tileVisuals.get("3,3");
...
for (const [key, visual] of tileVisuals) {
...
const actorVisual = tileVisuals.get("0,0");
if (actorVisual) {
...
const unexpectedVisuals = [...tileVisuals.values()].filter(
  (v) => v.affinityKind !== "fire" && v.affinityKind !== "water"
);
assert.equal(unexpectedVisuals.length, 0, "only fire and water visuals present");

console.log("affinity-pipeline-e2e: all assertions passed");
`;

test("affinity pipeline e2e: fixture → core → field records → tile visuals → renderer shape", (t) => {
