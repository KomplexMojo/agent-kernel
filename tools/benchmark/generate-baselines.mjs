...
function budgetFor(testCase) {
  return Number.isInteger(testCase?.budgetTokens) && testCase.budgetTokens > 0 ? testCase.budgetTokens : BUDGET;
}
...
function promptFor(testCase) {
  ...
  return [
    ...
    `${testCase.task}.`,
    `Use a ${budgetTokens} token hard budget, prefer MCP tools over shell commands, emit intermediates,`,
...
function main() {
  if (selectedCases.length !== 54) {
    throw new Error(`Expected 54 selected cases, found ${selectedCases.length}.`);
  }
...
These 54 prompts are intended for comparing how different LLMs and reasoning levels use the agent-kernel skill plus MCP tools. Each prompt file contains a copyable prompt, the reference \`ak_create\` payload, actual artifact file locations generated through the shared CLI/MCP implementation, and budget receipt cost tables.
