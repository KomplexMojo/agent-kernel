...
export function wireDesignGuidance({
  ...
  function adjustHazardManaValue(cardId, affinityKind, field, delta = 0) {
    const updated = updateCard(cardId, (card) => adjustHazardManaVital(card, affinityKind, field, delta));
    if (!updated) return false;
    recompute();
    return true;
  }
    ...
    if (front) {
      ...
      if (affinityList) {
        ...
        affinityEntries.forEach((entry) => {
          ...
          if (card.type === "room") {
            const manaMax = readNonNegativeInt(entry.hazardVitals?.mana?.max, 0);
            const manaRegen = readNonNegativeInt(entry.hazardVitals?.mana?.regen, 0);

            const manaControls = createDomElement(row, "div");
            if (manaControls) {
              manaControls.className = "design-card-hazard-mana-controls";
              ...
              if (maxMinus) {
                ...
                maxMinus.addEventListener?.("click", (event) => {
                  event.stopPropagation?.();
                  adjustHazardManaValue(card.id, entry.kind, "max", -5);
                });
              ...
              if (regenMinus) {
                ...
                regenMinus.addEventListener?.("click", (event) => {
                  event.stopPropagation?.();
                  adjustHazardManaValue(card.id, entry.kind, "regen", -1);
                });
  ...
  return {
    ...
    adjustHazardMana: adjustHazardManaValue,
    setPrimaryAffinity,
