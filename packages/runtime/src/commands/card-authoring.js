...
function stableSortAffinities(entries = []) {
  ...
  entries.forEach((entry) => {
      ...
      merged.set(key, { kind, expression, stacks, hazardVitals: entry?.hazardVitals });
    });
...
function adjustHazardManaVital(card, affinityKind, field, delta = 0) {
  const working = createDesignCard(card || {});
  ...
  const tv = entry.hazardVitals && typeof entry.hazardVitals === "object"
    ? { ...entry.hazardVitals }
    : {};
