 ...
 */
function resolveSolverEngine(env) {
  const value = env?.AK_SOLVER_ENGINE;
  ...
  return {
    ...
    host: {
      ...
      createSolverAdapter: async ({ fixturePath } = {}) => {
        if (!fixturePath && resolveSolverEngine(env) === "z3-real") {
          const { createHybridConstraintSolverAdapter } = await import("../z3/index.js");
          return createHybridConstraintSolverAdapter();
        }
