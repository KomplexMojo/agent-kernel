...
**Repo-owned skills** (`.claude/skills/`): `local-test-gen` (Ollama permutation expansion), `structured-test-authoring` (use the `ak_test_*` MCP tools before hand-writing a test), `tiered-test-optimizer` (the fast-pass → fix-pass recipe), persona-scoped change skills (`persona-orchestrator`, `persona-director`, `persona-configurator`, `persona-allocator`, `persona-actor`, `persona-moderator`, `persona-annotator`), and ports-and-adapters skills (`architecture-dependency-direction`, `architecture-effects-routing`, `architecture-artifacts-contracts`, `architecture-adapter-io`). Escalations from `fix-pass` reach the maintainer verbatim; the orchestrator never approves a boundary change itself.
