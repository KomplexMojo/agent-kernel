...
**Repo-owned skills** (canonical `.claude/skills/`, mirrored into `.cursor/skills/` for local Cursor discovery): `agentic-change-planning` (Plan/milestone routing onto a focused owner skill), `local-test-gen` (Ollama permutation expansion), `structured-test-authoring` (use the `ak_test_*` MCP tools before hand-writing a test), `tiered-test-optimizer` (the fast-pass → fix-pass recipe), persona-scoped change skills (`persona-orchestrator`, `persona-director`, `persona-configurator`, `persona-allocator`, `persona-actor`, `persona-moderator`, `persona-annotator`), ports-and-adapters skills (`architecture-dependency-direction`, `architecture-effects-routing`, `architecture-artifacts-contracts`, `architecture-adapter-io`), and `farm-remote` (run a benchmark or sweep on the remote Ollama GPU box; `disable-model-invocation: true`, so it is user-invoked only and never picked up automatically). ⚠️ `farm-remote` lived only under `.cursor/skills/` as a real directory until 2026-09-05 — Cursor could see it and Claude Code could not, and edits to it would have reached one harness and not the other, which is exactly the drift the symlink mirror exists to prevent. Every skill's canonical home is `.claude/skills/`; a real directory under `.cursor/skills/` is a defect, not a variant. Always-on Cursor rule `.cursor/rules/agentic-skill-routing.mdc` requires loading the matching skill when planning or implementing. Escalations from `fix-pass` reach the maintainer verbatim; the orchestrator never approves a boundary change itself.
