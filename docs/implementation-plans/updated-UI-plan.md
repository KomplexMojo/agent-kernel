...
Goal: Introduce a drag-and-drop card configurator that supports Room, Delver, and Warden setup using a shared left-rail item palette, a center card grid, and an AI-assisted alternative path. Preserve the approved trading-card visual style and enforce budget visibility and budget accounting directly on cards in real time.
...
- Left rail must expose draggable property groups in this order: Type (Room/Delver/Warden), Affinities, Expressions, Motivations.
- Center workspace must be a grid of blank card surfaces that accept dragged properties.
- Card configuration window must automatically group cards by type: Room, Delver, Warden.
...
- Delver cards support affinities, expressions, motivations, and vitals.
- Warden cards support affinities, expressions, motivations, and vitals.
...
- Card budget/value presentation must appear on all card types (Room, Delver, Warden).
...
1. Define a unified card schema for manual and AI configuration outputs.
   Requirement: A single normalized card model can represent Room, Delver, and Warden cards, including count, traits, vitals, and budget/value metadata.
...
2. Implement workspace layout and drag sources.
   Requirement: Single build workspace with left rail property groups (Type, Affinities, Expressions, Motivations), center card grid with blank surfaces, and card configuration grouping sections for Room/Delver/Warden.
...
4. Implement card creation, duplication, and count controls.
   ...
   Tests: Add count, deduplication, and auto-grouping tests in `tests/ui-web/design-view.test.mjs` for Room, Delver, and Warden cards.
   Code: `packages/ui-web/src/design-guidance.js`, `packages/ui-web/src/views/design-view.js`.
...
7. Add per-card budget/value display for all card types.
   Requirement: Room cards show real-time Card Value and shared level budget indicators; Delver/Warden cards show their configuration value and budget indicators aligned with pool rules on the card front.
   Tests: Add card HUD assertions in `tests/ui-web/design-view.test.mjs` for value updates and budget badges across Room/Delver/Warden cards.
...
10. Final integration and documentation sync.
   Requirement: Build-and-load flow works from card-derived configuration with automatic Room/Delver/Warden grouping in the card window, and plan docs reflect the new workflow.
