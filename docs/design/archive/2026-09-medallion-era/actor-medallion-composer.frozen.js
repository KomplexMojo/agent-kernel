// FROZEN 2026-09-02 — archived copy of packages/runtime/src/render/actor-medallion-composer.js.
// The original imported { AFFINITY_COLOR_HEX, hexToRgba } from "./affinity-palette.js", which
// chains into contracts/. That import is inlined below with the MEDALLION-ERA palette values so
// this file stays runnable after the live palette is re-derived. Do not "fix" these colours to
// match the current palette — they are the record of what the medallion actually looked like.
const AFFINITY_COLOR_HEX = Object.freeze({
  fire: "#f05a28", water: "#2b7fff", earth: "#7a5c33", wind: "#60d8c0", life: "#49b96b",
...
function hexToRgba(hex, alpha = 255) {
  ...
  return [parseInt(n.slice(1, 3), 16), parseInt(n.slice(3, 5), 16), parseInt(n.slice(5, 7), 16), alpha];
}
...
export const ACTOR_MEDALLION_COMPONENT_IDS = Object.freeze({
    ...
    affinities: Object.freeze(Object.fromEntries(Object.keys(AFFINITY_COLOR_HEX).map((kind) => [kind, `component.actor-medallion.affinity.${kind}`]))),
...
function setPixel(pixels, width, x, y, color) {
    if (x < 0 || y < 0 || x >= width || y >= pixels.length / 4 / width)
        return;
...
function drawLine(pixels, width, x0, y0, x1, y1, color, thickness = 1) {
...
function drawRing(pixels, width, cx, cy, outerRadius, innerRadius, color) {
...
function fillTriangle(pixels, width, p0, p1, p2, color) {
...
function blitScaled(target, targetSize, source, sourceSize, destX, destY, destSize, opacity = 1) {
    const alphaScale = clamp01(opacity);
    if (alphaScale <= 0 || source.length !== sourceSize * sourceSize * 4)
        return;
...
function drawFrame(pixels, size) {
...
function vitalFractionFromValue(value) {
...
function actorRecord(actor) {
    return actor && typeof actor === "object" ? actor : {};
}
function vitalFor(actor, key) {
...
function drawVitalBars(pixels, size, vitals) {
    const thickness = Math.max(2, Math.round(size * 0.078));
    const track = rgba("#060708", 230);
    const drawHorizontal = (y, fraction, color) => {
    ...
    const drawVertical = (x, fraction, color) => {
...
function firstAffinityEntry(actor) {
    const record = actorRecord(actor);
...
export function normalizeActorMedallionState(actor = {}, override = {}) {
...
function drawActorGlyph(pixels, size, role) {
    const s = size / 64;
...
function drawExpressionTriangles(pixels, size, expression) {
    const layouts = {
        push: [
            [[10, 5], [4, 2], [4, 8]],
            [[62, 5], [56, 2], [56, 8]],
    ...
    for (const triangle of layouts[expression]) {
        const shadow = triangle.map((point) => scalePoint([point[0] + 1, point[1] + 1], s));
        const foreground = triangle.map((point) => scalePoint(point, s));
...
function drawAffinityGlyph(pixels, size, state) {
    if (!state.affinity)
        return;
    const color = rgba(AFFINITY_COLOR_HEX[state.affinity] || "#ffffff");
    const glow = [color[0], color[1], color[2], 70];
    const s = size / 64;
    const cx = Math.round(32 * s);
...
function drawMotivationGlyph(pixels, size, motivation) {
    if (!motivation)
        return;
    const s = size / 64;
...
export function composeActorMedallion({ actor = {}, state, componentAtlas, size = ACTOR_MEDALLION_CANONICAL_SIZE, } = {}) {
    ...
    if (affinityLayer) {
        ...
        blitScaled(pixels, resolvedSize, affinityLayer, atlasSize, Math.round(resolvedSize / 2 - glyphSize / 2), Math.round(centerY - glyphSize / 2), glyphSize);
    }
    else {
...
export function buildActorMedallionComponentSprite(assetId, size = ACTOR_MEDALLION_CANONICAL_SIZE) {
    const resolvedSize = Math.max(8, Math.round(Number(size) || ACTOR_MEDALLION_CANONICAL_SIZE));
    const pixels = createPixelBuffer(resolvedSize, resolvedSize);
    ...
    if (id.startsWith("component.actor-medallion.vital.")) {
        const key = id.slice("component.actor-medallion.vital.".length);
        const fullVitals = {
        ...
        if (key in fullVitals)
            fullVitals[key] = { current: 1, max: 1, fraction: 1 };
